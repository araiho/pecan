...
read_restart.LINKAGES <- function(outdir, runid, stop.time, settings, var.names = NULL, params = NULL) {
  ...
  if ("Fcomp" %in% var.names) {
    forecast[[length(forecast)+1]] <- ens$AbvGrndWood #already has C  #* unit.conv 
    names(forecast[[length(forecast)]]) <- paste0('Fcomp.',pft.names)
  }
  
  if ("AbvGrndWood" %in% var.names) {
    forecast[[length(forecast)+1]] <- ens$AbvGrndWood #already has C  #* unit.conv 
    names(forecast[[length(forecast)]]) <- paste0('AbvGrndWood_',pft.names)
  }
  
  if ("GWBI" %in% var.names) {
    forecast[[length(forecast)+1]] <- udunits2::ud.convert(ens$GWBI,'kg/m^2/s','Mg/ha/yr') #udunits2::ud.convert(ens$TotSoilCarb, "kg/m^2", "Mg/ha") #* unit.conv 
    names(forecast[[length(forecast)]]) <- paste0("GWBI_",pft.names)
  }
