...
##' @param overwrite if TRUE Overwrite files
##' @export
##'
...
model2netcdf.LINKAGES <- function(outdir, sitelat, sitelon, start_date = NULL,
  ...
  for (y in seq_along(years)) {
    ...
    output[[6]]  <- npp.spp.save[,y,]  # GWBI = NPP in linkages
    output[[7]]  <- hetero.resp[y, ]  # HeteroResp in kgC/m^2/s
    output[[8]]  <- npp.spp.save[,y, ]  # NPP = GWBI in linkages
    ...
    output[[15]] <- agb.pft[, y, ] #AbvGroundWood just wood no leaves
    ...
    var[[6]]  <- ncdf4::ncvar_def("GWBI", "kgC/m2", list(dim.lat, dim.lon, dim.t,dim.pfts), -999)
    var[[7]]  <- ncdf4::ncvar_def("HeteroResp", "kgC/m2/s", list(dim.lat, dim.lon, dim.t), -999)
    var[[8]]  <- ncdf4::ncvar_def("NPP", "kgC/m2/s", list(dim.lat, dim.lon, dim.t,dim.pfts), -999)
    ...
    var[[15]] <- ncdf4::ncvar_def("AbvGrndWood", "kgC/m2", list(dim.lat, dim.lon, dim.t,dim.pfts), -999)
